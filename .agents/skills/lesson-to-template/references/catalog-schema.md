...
`staging.blank` используется только для диалога и сопоставляет существующее редактируемое служебное поле. На презентацию на утверждение (операция `build`) изображение ещё не встроено: сборщик оставляет заметку «СЦЕНА НЕ СГЕНЕРИРОВАНА» и не создаёт объект `DIALOGUE_SCENE`. Операция `embed-scenes` вставляет в копию утверждённого PPTX одно созданное полноэкранное изображение с этим именем для каждого диалогового слайда.
